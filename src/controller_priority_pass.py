"""Priority Pass traffic light controller FSM."""
...
class PriorityPassController:
    """Extend max-pressure auction control with priority bids from vehicles carrying UPP tokens.

    The winning phase is determined by a weighted combination of queue length and UPP bid,
    controlled by a configurable trade-off parameter.
    """
    ...
    def get_required_measurements(self) -> list[str]:
        ...
        Returns:
            List containing the queue measurement type and UPP bids.
        """
        ctrl = self.configuration.get(
            "priority_pass", self.configuration.get("control", {})
        )
        strategy = ctrl.get("bidding_strategy", "phase_queue_length")
        queue = "weighted_queue_lengths" if "weighted" in strategy else "queue_lengths"
        return [queue, "upp_bids"]
    ...
    def configure(self) -> "PriorityPassController":
        ...
        # support both "priority_pass" and legacy "control" keys for the auction parameters
        self.control = dict(
    ...
    def _transition(self, event: str) -> None:
        ...
        if next_state is None:
            raise RuntimeError(
                f"PriorityPassController cannot {event} from {self.state}"
            )
    ...
    def _get_priority_pass_bids(self, metrics: dict[str, Any]) -> list[float]:
        """Compute the combined priority-pass bid for each phase.

        Blends the queue-length bid and the UPP priority bid using the configured
        trade-off parameter tau: bid = (1 - tau) * queue_bid + tau * upp_bid.

        Args:
            metrics: Measurement data for one traffic light.
        Returns:
            List of combined bid values, one per phase.
        """
        ...
        # fall back to zero if UPP bids not provided (e.g. no priority vehicles present)
        upp_bids = list(metrics.get("upp_bids", [0.0 for _ in phase_bids]))
        tau = float(self.control.get("trade_off", 0.0))

        # blend queue bid and UPP bid: bid = (1-tau)*queue_bid + tau*upp_bid
        bids = []
        for i, phase_bid in enumerate(phase_bids):
            upp_bid = float(upp_bids[i]) if i < len(upp_bids) else 0.0
