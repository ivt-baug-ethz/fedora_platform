...
#
# Tracking = .ai/ (local progress log) and .serena/memories/ (reference knowledge,
# incl. the committed architectural_decisions ADR log). Fires when the working tree
# has changes OUTSIDE those (and outside
...
if [ -n "$work" ] && [ -z "$tracking_touched" ]; then
  cat <<'JSON'
{"decision":"block","reason":"There are uncommitted changes but project tracking was not updated this session. Per AGENTS.md: append a dated entry to .ai/PROGRESS.md (always), update the relevant serena memory via write_memory if structure/conventions/commands changed, and add an ADR to the architectural_decisions serena memory for notable choices; then finish. If the changes genuinely warrant no tracking update, add a one-line note in .ai/PROGRESS.md saying so (or commit the work), then finish."}
JSON
fi
exit 0
